...
addVariable :: String -> IGraph -> IGraph
...
--   added to the graph first if not already there, then each is added to
--   the other's adjacency list.
addEdge :: String -> String -> IGraph -> IGraph
addEdge name1 name2 graph  =
    let graph'              = addVariable name1 (addVariable name2 graph)
        IGraph vars'        = graph'
